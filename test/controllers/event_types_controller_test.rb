...
class EventTypesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create event_type" do
    assert_difference("EventType.count") do
      post event_types_url, params: { event_type: { color: @event_type.color, name: @event_type.name } }
    end
  ...
  test "should update event_type" do
    patch event_type_url(@event_type), params: { event_type: { color: @event_type.color, name: @event_type.name } }
